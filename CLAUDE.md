...
- `shared/sfx.js` — WebAudio 合成の共通効果音(`SFX.tap()` 等。音声アセット不要)
- `lib/three/` — three.js の vendoring(3D ゲーム用、ES Modules で import)
- `signal/` — 全ゲームクリア後の最終コンテンツ
...
- 履歴には意図的な不自然さを混ぜてよい(無意味なコミット分割、同一メッセージの反復など)。
  ただしメッセージの文面自体にネタバレは書かない
- **この節は全ゲーム完成時に丸ごと削除すること**
